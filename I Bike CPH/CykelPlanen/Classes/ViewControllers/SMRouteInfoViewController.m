...
- (void)filterLines
{
    SMTransportation *transportation = [SMTransportation sharedInstance];
    ...
    if (self.singleRouteInfo.type == SMStationInfoTypeLocalTrain) {
        ...
        for (SMTrain *train in trains) {
            NSArray *array = [train routeTimestampsForSourceStation:self.singleRouteInfo.sourceStation
                                                 destinationStation:self.singleRouteInfo.destStation
                                                             forDay:weekday
                                                               time:cTime];
            if (array) {
        ...
        [timesArray sortUsingComparator:^NSComparisonResult(SMRouteTimeInfo *t1, SMRouteTimeInfo *t2) {
          ...
          NSInteger diff1 = [cTime differenceInMinutesFrom:src];
          NSInteger diff2 = [cTime differenceInMinutesFrom:src2];
          if (diff1 > diff2)
        ...
        do {
            ...
            for (NSInteger i = 0; i < ((NSInteger)timesArray.count) - 1; i++) {
                SMRouteTimeInfo *first = timesArray[i];
                for (NSInteger j = i + 1; j < timesArray.count; j++) {
        ...
        NSString *urlString = [NSString
            stringWithFormat:@"https://xmlopen.rejseplanen.dk/bin/rest.exe/location?input=%@", [self.singleRouteInfo.sourceStation.name urlEncode]];
    ...
    /*
        ...
        if([self isNightForDayAtIndex:6 dayIndex:[weekdayComponents weekday] hour:cTime.hour] || [self isNightForDayAtIndex:7
    dayIndex:[weekdayComponents weekday] hour:cTime.hour]){
        ...
        NSInteger diff= [self.singleRouteInfo.transportationLine differenceFrom:self.singleRouteInfo.sourceStation to:self.singleRouteInfo.destStation];
        ...
        NSInteger minutesBetweenDeparture = 2;
        NSInteger h = firstTime.hour;
        if (startIndex < 7) {
        ...
        double metroTimingConst[] =
    {0.7335,1.4172,1.0357,0.9220,2.0112,1.9194,1.4138,0.9644,1.7651,0.8496,1.1164,0.7781,0.9434,1.0419,5.1643,1.5944,1.3921,0.6224,1.2332,1.3312,1.0685};
    //,0.0000};
        ...
        for(NSInteger i=0; i<3; i++){
            ...
            if (startIndex < endIndex) {
                for (NSInteger st=startIndex; st<endIndex; st++) {
                ...
                for (NSInteger st=endIndex; st<startIndex; st++) {
                    travelTime += averageTimePerStation * metroTimingConst[st] * magicNumber;
                }
            }
            ...
            SMRouteTimeInfo* routeTimeInfo= [[SMRouteTimeInfo alloc] initWithRouteInfo:self.singleRouteInfo sourceTime:sTime
...
- (BOOL)isNightForDayAtIndex:(NSInteger)index dayIndex:(NSInteger)dayIndex hour:(NSInteger)hour
{
    return (dayIndex == index && hour > 20) || (((dayIndex + 1) % 7) == index && hour < 5);
}

- (BOOL)isDayForDayAtIndex:(NSInteger)index components:(NSDateComponents *)comps
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row) {
        case 0: {
            ...
            [lbl setText:[NSString
                             stringWithFormat:@"%@, %@ %@", [dateFormatter stringFromDate:[NSDate new]], @"departures_at".localized, currentTime]];
         ...
         attributes:(NSDictionary *)attributeDict
{
    if (parser == idParser && [elementName isEqualToString:@"StopLocation"]) {
        ...
        if ([elementName isEqualToString:@"Leg"]) {
            ...
            if (currentTimeInfo) currentTimeInfo.sourceTime = [SMTime timeFromString:[attributeDict objectForKey:@"time"]];
        }
