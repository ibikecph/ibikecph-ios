...
typedef enum { AddressTypeUndefined = -1, AddressTypeSource = 0, AddressTypeDestination = 1 } AddressType;

@class SMAddressPickerView;

@protocol AddressSelectDelegate<NSObject>

- (void)addressView:(SMAddressPickerView *)addressPickerView didSelectItemAtIndex:(NSInteger)index forAddressType:(AddressType)pAddressType;
- (NSString *)addressView:(SMAddressPickerView *)addressPickerView titleForRow:(NSInteger)row;
@end
