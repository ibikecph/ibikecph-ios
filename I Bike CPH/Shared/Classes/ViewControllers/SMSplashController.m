...
- (IBAction)goToFavorites:(id)sender {
    ...
    if(![[SMReminder sharedInstance] isReminderScreenShown]){
        [self performSegueWithIdentifier:@"loginToReminder" sender:nil];
    } else if ([[SMFavoritesUtil getFavorites] count] > 0) {
        [self performSegueWithIdentifier:@"splashToMain" sender:nil];
    } else {
...
- (void)request:(SMAPIRequest *)req completedWithResult:(NSDictionary *)result {
    if ([result[@"success"] boolValue]) {
        ...
        NSString *token = data[@"auth_token"];
        NSString *identifier = data[@"id"];
        BOOL hasToken = token && ![token isKindOfClass:[NSNull class]];
        if ([req.requestIdentifier isEqualToString:@"login"]) {
            [self.appDelegate.appSettings setValue:token forKey:@"auth_token"];
            [self.appDelegate.appSettings setValue:identifier forKey:@"id"];
            ...
            [self fetchFavs];
                ...
                [self.appDelegate saveSettings];
                [self fetchFavs];
        } else if ([req.requestIdentifier isEqualToString:@"loginFB"] && hasToken) {
            if(hasToken) {
                [self.appDelegate.appSettings setValue:token forKey:@"auth_token"];
            }
            [self.appDelegate.appSettings setValue:identifier forKey:@"id"];
