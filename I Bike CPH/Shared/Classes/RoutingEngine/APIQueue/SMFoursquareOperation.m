    ...
    if (near) {
        URLString =
            [NSString stringWithFormat:@"https://api.foursquare.com/v2/venues/"
                                       @"search?intent=browse&near=%@&client_id=%@&client_secret=%@&query=%@&v=%@&radius=%@&limit=%@&categoryId=%@",
                                       [[near removeAccents] urlEncode], FOURSQUARE_ID, FOURSQUARE_SECRET,
                                       [[self.searchString removeAccents] urlEncode], @"20130301", FOURSQUARE_SEARCH_RADIUS,
                                       [SMRouteSettings sharedInstance].foursquare_limit, [SMRouteSettings sharedInstance].foursquare_categories];
    }
    else {
        URLString =
            [NSString stringWithFormat:@"https://api.foursquare.com/v2/venues/"
                                       @"search?intent=browse&ll=%f,%f&client_id=%@&client_secret=%@&query=%@&v=%@&radius=%@&limit=%@&categoryId=%@",
                                       [SMLocationManager sharedInstance].lastValidLocation.coordinate.latitude,
                                       [SMLocationManager sharedInstance].lastValidLocation.coordinate.longitude, FOURSQUARE_ID, FOURSQUARE_SECRET,
                                       [[self.searchString removeAccents] urlEncode], @"20130301", FOURSQUARE_SEARCH_RADIUS,
                                       [SMRouteSettings sharedInstance].foursquare_limit, [SMRouteSettings sharedInstance].foursquare_categories];
    }
            ...
            if (d[@"categories"] && [d[@"categories"] count] > 0) {
                ...
                if (d2[@"icon"]) {
                    NSString *s1 = [NSString
                        stringWithFormat:@"%@%@", [d2[@"icon"][@"prefix"]
