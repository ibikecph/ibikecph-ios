...
- (void)findPlacesForLocation:(CLLocation *)loc
{
    self.coord = loc;
    NSString *s = [NSString
        stringWithFormat:@"https://geo.oiorest.dk/adresser/%f,%f,%@.json", loc.coordinate.latitude, loc.coordinate.longitude, OIOREST_SEARCH_RADIUS];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    if ([self.responseData length] > 0) {
        ...
        id res = [NSJSONSerialization JSONObjectWithData:self.responseData
                                                 options:NSJSONReadingAllowFragments
                                                   error:nil];  //[[[SBJsonParser alloc] init] objectWithData:self.responseData];
