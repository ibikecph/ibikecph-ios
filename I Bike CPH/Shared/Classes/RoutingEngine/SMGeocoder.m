    ...
    [NSURLConnection sendAsynchronousRequest:req
                           ...
                           completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
                             ...
                             if (error) {
                                 ...
                                 handler(@[], [NSError errorWithDomain:NSOSStatusErrorDomain
                                                                  code:1
                                                              userInfo:@{
...
+ (void)kortPlaceGeocode:(NSString *)str completionHandler:(void (^)(NSArray *placemarks, NSError *error))handler
{
    NSString *URLString = [[NSString
        stringWithFormat:@"https://kortforsyningen.kms.dk/"
                         ...
                         KORT_SERVICE, str, [SMLocationManager sharedInstance].lastValidLocation.coordinate.longitude,
                           ...
                           completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
                             ...
                             if (error) {
                                 ...
                                 handler(@[], [NSError errorWithDomain:NSOSStatusErrorDomain
                                                                  code:1
                                                              userInfo:@{
                                                                  NSLocalizedDescriptionKey : @"Wrong data returned from the KMS"
                                                              }]);
                             }
                             else {
                                 ...
                                 for (NSDictionary *feature in json[@"features"]) {
                                     ...
                                     NSString *formattedAddress = [[NSString stringWithFormat:@"%@ %@, %@ %@", item.street, item.number, item.zip,
                                                                                              item.city] stringByTrimmingCharactersInSet:set];
                                     item.name = formattedAddress;
                                     item.address = formattedAddress;

                                     NSDictionary *dict = @{(NSString *)kABPersonAddressStreetKey : item.name};
                                     MKPlacemark *pl = [[MKPlacemark alloc] initWithCoordinate:item.location.coordinate addressDictionary:dict];
                                     [arr addObject:pl];
                                 }
                                 handler(arr, nil);
...
+ (void)kortAddressGeocode:(NSObject<SearchListItem> *)item completionHandler:(void (^)(NSArray *placemarks, NSError *error))handler
{
    ...
    NSString *URLString = [
        [NSString stringWithFormat:@"https://kortforsyningen.kms.dk/"
                                   @"?servicename=%@&method=adresse&%@&geop=%lf,%lf&georef=EPSG:4326&outgeoref=EPSG:4326&login=%@&password=%@&hits=%"
                                   @"@&geometry=true",
...
{
    CLGeocoder *cl = [[CLGeocoder alloc] init];
    [cl reverseGeocodeLocation:[[CLLocation alloc] initWithLatitude:coord.latitude longitude:coord.longitude]
             completionHandler:^(NSArray *placemarks, NSError *error) {
               NSString *title = @"";
               NSString *subtitle = @"";
               NSMutableArray *arr = [NSMutableArray array];
               if ([placemarks count] > 0) {
                   ...
                   for (MKPlacemark *localPlacemark in placemarks) {
                       ...
                       [arr addObject:@{ @"street" : street, @"house_number" : @"", @"zip" : zip, @"city" : city }];
                   }
               }
                ...
                if (error) {
                    ...
                    if (data) {
                            ...
                            handler(@{}, [NSError errorWithDomain:NSOSStatusErrorDomain
                                                         ...
                                                         userInfo:@{
                                                             NSLocalizedDescriptionKey :
                                                                 [NSString stringWithFormat:@"Wrong data returned from the OIOREST: %@", s]
                                                         }]);
                            return;
                        }
                        if ([res isKindOfClass:[NSArray class]] == NO) {
                        ...
                        for (NSDictionary *d in res) {
                            OiorestItem *item = [[OiorestItem alloc] initWithJsonDictionary:d];
                            [arr addObject:@{ @"street" : item.street, @"house_number" : item.number, @"zip" : item.zip, @"city" : item.city }];
                        }
                        handler(@{ @"title" : title, @"subtitle" : subtitle, @"near" : arr }, nil);
                    }
                    else {
